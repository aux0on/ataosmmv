...
do
    ...
    local pfUpdateConnection = nil

    local function DestroyPF()
        if pfUpdateConnection then pfUpdateConnection:Disconnect() pfUpdateConnection = nil end
        if pfGui and pfGui.Parent then pfGui:Destroy() pfGui = nil end
        pfFrame = nil
        pingLabel = nil
        fpsLabel = nil
    end

    local function CreatePFDisplay()
        DestroyPF()

        pfGui = Instance.new("ScreenGui", LocalPlayer:WaitForChild("PlayerGui"))
        pfGui.Name = "PFGui"
        pfGui.ResetOnSpawn = false

        pfFrame = Instance.new("Frame", pfGui)
        pfFrame.Name = "PFFrame"
        pfFrame.Size = UDim2.new(0, 120, 0, 50)
        pfFrame.Position = UDim2.new(1, -130, 0, 10)
        pfFrame.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
        pfFrame.BackgroundTransparency = 0.3
        pfFrame.BorderSizePixel = 0
        Instance.new("UICorner", pfFrame).CornerRadius = UDim.new(0, 6)

        local stroke = Instance.new("UIStroke", pfFrame)
        stroke.Thickness = 2
        stroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
        local gradient = Instance.new("UIGradient", stroke)
        gradient.Color = ColorSequence.new{
        ...
        pingLabel = Instance.new("TextLabel", pfFrame)
        ...
        fpsLabel = Instance.new("TextLabel", pfFrame)
        ...
        pfUpdateConnection = RunService.RenderStepped:Connect(function(dt)
            if not pfEnabled then return end

            if pingLabel and pingLabel.Parent and pingEnabled then
            ...
            if lastFpsUpdate >= 0.5 then
                lastFpsUpdate = 0
                if fpsLabel and fpsLabel.Parent and fpsEnabled then
    ...
    pfSection:AddToggle("Display P&F", function(e)
        pfEnabled = e
        if e then
            CreatePFDisplay()
        else
    ...
    pfSection:AddToggle("Enable Ping Display", function(e)
        pingEnabled = e
    ...
    pfSection:AddToggle("Enable FPS Display", function(e)
        fpsEnabled = e
